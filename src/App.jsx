...
function Button({ className = "", variant, size, disabled, ...props }) {
  const base =
    "inline-flex items-center justify-center rounded-md px-4 py-2 text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-400 disabled:opacity-60 disabled:cursor-not-allowed";
  const variants = {
  ...
  const updates = [
    {
      ...
      title: "Fixes & polish",
      items: [
        "Community page Layout (single footer)",
        "Bigger hit areas for user menu",
        "Docs page added",
      ],
    },
    {
      ...
      title: "Live",
      items: [
        "Hero, features, stats, and pricing",
    ...
    <>
      ...
      <div className="min-h-screen bg-[#0b0c10] text-white">
        ...
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <div
            className="absolute -top-24 left-1/2 h-72 w-[56rem] -translate-x-1/2 rounded-full blur-3xl opacity-30"
            style={{ background: "radial-gradient(closest-side, rgba(130,87,229,.6), rgba(0,0,0,0))" }}
          />
          <div
            className="absolute -bottom-24 left-1/3 h-72 w-[48rem] -translate-x-1/3 rounded-full blur-3xl opacity-25"
            style={{ background: "radial-gradient(closest-side, rgba(37,99,235,.55), rgba(0,0,0,0))" }}
          />
        </div>

        {/* header (hide on control) */}
        <header className={`relative z-10 ${page === "control" ? "hidden" : ""}`}>
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-5 flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-8 text-sm text-white/70">
              <button className="hover:text-white transition" onClick={() => setPage("home")}>
                Home
              </button>
              <a
              ...
              <button className="hover:text-white transition" onClick={() => setPage("docs")}>
                Docs
              </button>
            </nav>
            <div className="flex items-center gap-3 relative">
              {!user ? (
                <>
                  <Button variant="ghost" className="text-white/80 hover:text-white" onClick={() => setPage("login")}>
                    Log in
                  </Button>
                  <Button className="bg-indigo-500 hover:bg-indigo-400" onClick={() => setPage("download")}>
                    Download
                  </Button>
                </>
              ) : (
                <div className="relative">
                  ...
                  >
                    ...
                    <svg width="16" height="16" viewBox="0 0 20 20" className="opacity-80">
                  ...
                  {menuOpen && (
                    <div className="absolute right-0 mt-2 w-56 rounded-xl border border-white/10 bg-[#0b0c10] shadow-[0_0_22px_rgba(99,102,241,.35)]">
                      <button className="block w-full text-left px-4 py-3 text-sm hover:bg-white/5" onClick={() => { setPage("control"); setMenuOpen(false); }}>
                        Control Panel
                      </button>
                      <button className="block w-full text-left px-4 py-3 text-sm hover:bg-white/5" onClick={() => { setPage("community"); setMenuOpen(false); }}>
                        Community
                      </button>
                      <button className="block w-full text-left px-4 py-3 text-sm hover:bg-white/5" onClick={() => { setPage("settings"); setMenuOpen(false); }}>
                        Settings
                      </button>
                      <button className="block w-full text-left px-4 py-3 text-sm text-red-300 hover:bg-white/5" onClick={() => { setUser(null); setMenuOpen(false); showToast("Signed out"); }}>
                        Sign out
                      </button>
                    </div>
                  )}
          ...
          <>
            <section className="relative z-10">
              <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-16 md:py-24 flex flex-col items-center text-center">
                ...
                {/* stats with same glow as download cards */}
              ...
              <div className="mt-8 grid md:grid-cols-3 gap-6">
                {[
                  ...
                  <Card key={d.name} className="bg-white/10 border-white/20 backdrop-blur-md shadow-[0_0_35px_rgba(99,102,241,.35)] hover:shadow-[0_0_55px_rgba(99,102,241,.55)] transition pulse-glow">
                    <CardHeader>
                      <CardTitle className="text-indigo-300 drop-shadow">{d.name}</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-white text-sm font-medium">{d.note}</p>
                      <Button disabled className="mt-4 w-full bg-indigo-500 hover:bg-indigo-500/90 shadow-[0_0_30px_rgba(99,102,241,.45)] cursor-not-allowed">
                        Coming soon
                      </Button>
                      <button onClick={() => setPage("login")} className="mt-3 w-full text-center text-indigo-200/95 hover:text-white underline text-xs">
                        Join the waitlist
                      </button>
                    </CardContent>
                  </Card>
          ...
          <section className="relative z-10 py-16">
            <div className="mx-auto max-w-md px-4 sm:px-6 lg:px-8">
              ...
              <p className="text-white/70 mt-1 text-sm">
                Log in or create an account to continue. New? Create an account below — we'll capture your email automatically.
              </p>
              <Card className="mt-6 bg-white/5 border-white/10">
                <CardContent className="pt-6">
                  ...
                  <form
                    ...
                    onSubmit={(e) => {
                      e.preventDefault();
                      const email = (e.currentTarget.querySelector('input[placeholder="you@example.com"]') || {}).value || "user@ovara.app";
                      setUser({ email });
                      setPage("home");
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <label className="flex items-center gap-2 text-xs text-white/70">
                        <input type="checkbox" className="accent-indigo-500" /> Keep me signed in
                      </label>
                      <a className="text-xs text-indigo-300 hover:text-indigo-200">Forgot password?</a>
                    </div>
                    <Button className="w-full bg-indigo-500 hover:bg-indigo-400" type="submit">
                      Sign in
                    </Button>
                  </form>
                  ...
                  <div className="mt-8 border-t border-white/10 pt-6">
                    ...
                    <form
                      ...
                      onSubmit={(e) => {
                        ...
                        try {
                          const list = JSON.parse(localStorage.getItem("ovara_users") || "[]");
                          list.push({ email, createdAt: Date.now() });
                          localStorage.setItem("ovara_users", JSON.stringify(list));
                          const wl = JSON.parse(localStorage.getItem("ovara_waitlist") || "[]");
                          wl.push({ email, from: "signup", createdAt: Date.now() });
                          localStorage.setItem("ovara_waitlist", JSON.stringify(wl));
                        } catch {}
        ...
        {page === "community" && (
          <>
                ...
                <Card className="bg-white/10 border-white/20 backdrop-blur-md shadow-[0_0_35px_rgba(99,102,241,.45)]">
                  ...
                  <CardContent>
                    <p className="text-white/80 text-sm">
                      Hop into our Discord to get updates, report issues, and meet other users. Click below to join.
                    </p>
                    <Button
                      className="mt-4 w-full"
                      onClick={() => {
                        const url = localStorage.getItem("ovara_discord_url") || "#";
                        if (url === "#") {
                          showToast("Set ovara_discord_url in localStorage to enable");
                        } else {
            ...
            {/* page-specific footer below card */}
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <ul className="mt-2 space-y-1">
                      <li>
                        <button className="hover:text-white" onClick={() => setPage("download")}>
                          Download
                        </button>
                      </li>
                      <li>
                        <button className="hover:text-white" onClick={() => setPage("docs")}>
                          Changelog
                        </button>
                      </li>
                      <li>
                  ...
                  <div>
                    ...
                    <ul className="mt-2 space-y-1">
                      ...
                      <li>
                        <button className="hover:text-white" onClick={() => setPage("community")}>
                          Community
                        </button>
                      </li>
                    </ul>
                  ...
                  <Card key={u.version} className="bg-white/10 border-white/20">
                    <CardHeader>
                      <CardTitle className="text-indigo-200">
                        {u.version} — {u.title}{" "}
                        <span className="text-xs text-white/50 ml-2">{u.date}</span>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
        ...
        {/* GLOBAL FOOTER (hidden on control/community) */}
